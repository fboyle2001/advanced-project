...
pjoin = lambda a, b, c: _pjoin(a, b, c).replace("\\", "/")

CONFIGS = {
    # "sup_vitb8": configs.get_b16_config(),
    "sup_vitb16_224": vit_configs.get_b16_config(),
    "sup_vitb16": vit_configs.get_b16_config(),
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        self.position_embeddings = nn.parameter.Parameter(torch.zeros(1, n_patches+1, config.hidden_size))
        self.cls_token = nn.parameter.Parameter(torch.zeros(1, 1, config.hidden_size))
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.head = Linear(config.hidden_size, num_classes) if num_classes > 0 else nn.Identity() # type: ignore
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                self.transformer.embeddings.hybrid_model.root.conv.weight.copy_(np2th(weights["conv_root/kernel"], conv=True)) # type: ignore
                gn_weight = np2th(weights["gn_root/scale"]).view(-1)
                gn_bias = np2th(weights["gn_root/bias"]).view(-1)
                self.transformer.embeddings.hybrid_model.root.gn.weight.copy_(gn_weight) # type: ignore
                self.transformer.embeddings.hybrid_model.root.gn.bias.copy_(gn_bias) # type: ignore

                for bname, block in self.transformer.embeddings.hybrid_model.body.named_children():
                    for uname, unit in block.named_children():
                        unit.load_from(weights, n_block=bname, n_unit=uname) # type: ignore
    ...
    def __init__(self, block_units, width_factor):
        ...
        self.body = nn.Sequential(OrderedDict([
            ('block1', nn.Sequential(OrderedDict(
                [('unit1', PreActBottleneck(cin=width, cout=width*4, cmid=width))] +  # type: ignore
                [(f'unit{i:d}', PreActBottleneck(cin=width*4, cout=width*4, cmid=width)) for i in range(2, block_units[0] + 1)], 
                ))),
            ('block2', nn.Sequential(OrderedDict(
                [('unit1', PreActBottleneck(cin=width*4, cout=width*8, cmid=width*2, stride=2))] +  # type: ignore
                [(f'unit{i:d}', PreActBottleneck(cin=width*8, cout=width*8, cmid=width*2)) for i in range(2, block_units[1] + 1)],
                ))),
            ('block3', nn.Sequential(OrderedDict(
                [('unit1', PreActBottleneck(cin=width*8, cout=width*16, cmid=width*4, stride=2))] +  # type: ignore
