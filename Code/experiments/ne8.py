...
class NovelExperimentEight(BaseCLAlgorithm):
    """
    Experiment 8: Freeze ViT and only train the MLP head with CE loss
    
    CIFAR-10  (2000 samples) final accuracy: 
    CIFAR-100 (5000 Samples) final accuracy: 73.36%
    """
    def __init__(
        self,
        ...
        super().__init__(
            name="Novel Experiment: Idea Eight",
    ...
    def get_algorithm_folder() -> str:
        return "novel_experiment/idea_eight"
        ...
        for task_no, (task_indices, task_dataset) in enumerate(zip(self.dataset.task_splits, self.dataset.raw_task_datasets)):
            ...
            for epoch in range(self.epochs_per_task):
                for batch_no, data in enumerate(buffer_dl, 0):
                    ...
                    predictions = self.pretrained_vit(inp)
    ...
    def generate_embeddings(self):
        ...
        if not self.require_mean_calculation:
            logger.warning("Not generated as not needed")
            return
