...
class NovelExperimentOne(BaseCLAlgorithm):
    ...
    These are then used with Nearest Feature Embedding classification
    - maybe try downsizing the features?
            ...
            for epoch in range(1, self.epochs_per_task + 1):
                ...
                for batch_no, data in enumerate(task_dataloader, 0):
                    ...
                    if self.buffer.count >= self.memory_batch_size:
                        ...
                        clustered_features = self.model.features(vit_features)
    ...
    def classify(self, batch: torch.Tensor) -> torch.Tensor:
        if self.require_mean_calculation:
            ...
            for target in range(0, len(self.dataset.classes)):
                if target not in self.buffer.known_classes:
                    means.append(torch.zeros(1, self.reduced_D).to("cpu"))
                    continue
                ...
                for batch_no in range(0, buffered.shape[0] // batch_size + 1):
                    ...
                    vit_encoded = self.f_r(self.f_e(buffer_batch))[:, 0, :]
                    encoded = self.model.features(vit_encoded).detach().clone()
        ...
        vit_features = self.f_r(self.f_e(batch))[:, 0, :]
        features = self.model.features(vit_features)
        tiled = features.tile(dims=(1, C)).reshape(B, C, self.reduced_D)
...
class TestNet(nn.Module):
    def __init__(self):
        ...
        self.linear_one = nn.Linear(768, 512)
        self.linear_two = nn.Linear(512, 512)
        self.linear_three = nn.Linear(512, 256)
        self.linear_four = nn.Linear(256, 256)
