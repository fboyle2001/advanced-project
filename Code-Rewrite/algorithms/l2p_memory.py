...
class LearningToPromptWithMemory(BaseCLAlgorithm):
    ...
    def __init__(
        ...
        self.buffer = buffers.BalancedReplayBuffer(len(self.dataset.classes) * 50)
    ...
    def train(self) -> None:
        ...
        for task_no, (task_indices, task_dataset) in enumerate(zip(self.dataset.task_splits, self.dataset.task_datasets)):
            ...
            if task_no > 0 and self.prompt_frequency_strategy != "disabled":
                ...
                exemplar_dataset = self.buffer.to_torch_dataset(self.dataset.training_transform)
            ...
            for epoch in range(1, self.epochs_per_task + 1):
                ...
                # Loop the training data - technically breaks online but lazy way of testing this initially
                for index in range(len(task_dataset)): # type: ignore
                    img = task_dataset.data[index] # type: ignore
                    target = task_dataset.targets[index].item() # type: ignore
                    
                    # Create a balanced replay buffer of samples
                    self.buffer.add_sample(img, target)
