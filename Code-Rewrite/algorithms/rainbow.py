...
class Rainbow(BaseCLAlgorithm):
    ...
    def perform_diversity_aware_memory_update(self, exemplars, exemplar_targets, task_samples, task_targets):
        ...
        for class_name in class_segmented_samples.keys():
            ...
            for j in range(memory_per_class):
                sample_idx = (num_class_samples * j) // memory_per_class
    ...
    def train(self) -> None:
        ...
        for task_no in range(len(self.dataset.task_datasets)):
            ...
            non_exemplar_batch_size = self.batch_size
            exemplar_dataloader = None
            exemplar_iter = None

            if len(exemplar_data) != 0:
                logger.info("Using memory buffer, halved batch")
                exemplar_dataset = CustomImageDataset(exemplar_data, exemplar_targets, transform=self.dataset.training_transform)
                exemplar_dataloader = DataLoader(exemplar_dataset, batch_size=self.batch_size // 2, shuffle=True)
                exemplar_iter = iter(exemplar_dataloader)

                non_exemplar_batch_size = non_exemplar_batch_size // 2
            else:
            ...
            for epoch in range(1, self.epochs_per_task + 1):
                ...
                for batch_no, data in enumerate(non_exemplar_dataloader):
                    # Need to draw from both dataloaders if they are available
                ...
                # Metrics
                avg_running_loss = running_loss / (len(non_exemplar_dataloader) - 1)
